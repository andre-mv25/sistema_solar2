(function(){
  ...
  for(var i=0;i<planetas.length;i++){
    if(i<radios.length){
      var o=document.createElement('div');o.className='orbita';o.style.width=radios[i]*2+'px';o.style.height=radios[i]*2+'px';sist.appendChild(o);
    } else {
      var r=150+(i-7)*22;
      var o=document.createElement('div');o.className='orbita';o.style.width=r*2+'px';o.style.height=r*2+'px';sist.appendChild(o);
    }
  }
  ...
  var builtinOrbitas=[6,9,12,15,20,24,28,33];
  function getOrbita(i){
    if(i<builtinOrbitas.length) return builtinOrbitas[i];
    return 33+(i-7)*5;
  }
  planetas.forEach(function(p,i){
    var o=document.createElement('div');o.className='p-orbita';
    o.style.setProperty('--o',(20+i*10)+'s');o.style.setProperty('--r',getOrbita(i)+'px');
    var b=document.createElement('a');b.className='p-bola';
    b.href=i<builtinCount?(base+'planetas/'+p.slug+'.html'):(base+'planeta.html?slug='+p.slug);
  ...
  if(grid){
    planetas.forEach(function(p,i){
      var c=document.createElement('a');c.className='card';
      c.href=i<builtinCount?(base+'planetas/'+p.slug+'.html'):(base+'planeta.html?slug='+p.slug);
  ...
  if(drop){
    planetas.forEach(function(p,i){
      ...
      a.href=i<builtinCount?(base+'planetas/'+p.slug+'.html'):(base+'planeta.html?slug='+p.slug);
